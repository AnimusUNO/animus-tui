...
from animaos.knowledge.base import KnowledgeNotFound, default_knowledge_base
...
class AnimaOSApp(App[None]):
    ...
    BINDINGS = [
        ...
        Binding("ctrl+k", "toggle_knowledge", "Knowledge"),
    ]
    ...
    def __init__(self) -> None:
        ...
        self._knowledge_only = False
        self._knowledge = default_knowledge_base()
    ...
    def _home_screen_text(self) -> str:
        commands = (
            ...
            "/knowledge toggle local knowledge\n"
            "/quit     exit"
        )
    ...
    async def _handle_command(self, command_line: str) -> None:
        ...
        if command == "help":
            self._emit_system("Commands: /help /status /agents /agent <id> /knowledge /clear /quit")
        elif command == "status":
            agent = letta_client.current_agent_id or "(none)"
            mode = "local" if self._knowledge_only else "relay"
            ...
            self.action_toggle_knowledge()
            mode = "local" if self._knowledge_only else "relay"
            self._emit_system(f"Knowledge mode set to {mode}.")
        elif command == "quit":
            self.exit()
        else:
    ...
    async def _respond(self, text: str) -> None:
        if self._knowledge_only:
            try:
                answer = self._knowledge.answer(text)
                self._transcript.append("animaos", answer)
            except KnowledgeNotFound:
                self._transcript.append("animaos", "No local knowledge available.")
            return
    ...
    def action_toggle_knowledge(self) -> None:
        self._knowledge_only = not self._knowledge_only

    def action_show_help(self) -> None:
        self._emit_system(
            "Keyboard: ctrl+n new, ctrl+h help, ctrl+a agents, ctrl+m models, ctrl+k knowledge, ctrl+q quit"
        )
